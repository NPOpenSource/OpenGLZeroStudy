...
#import "EffectOneViewController.h"
#import "MoreObjectBindObject.h"
#import "EffectVertex.h"

@interface EffectOneViewController ()
...
@implementation EffectOneViewController

-(void)initSubObject{
    self.cacheVertexArr = [NSMutableArray array];
    self.eachInterval = 1.0;
    //生命周期三秒钟
    __weakSelf
...
-(void)updateVertexForce:(EffectVertex *)vertex{
    ...
    vertex.force = GLKVector3Make( randomXVelocity
                               , randomYVelocity, 0);
...
-(void)createVertex{
    for(int i = 0; i < 100; i++)
    {
        ...
        bindAtt.baseBindAttrib.beginPosition =GLKVector3Make(0.0f, 0.0, 0.9);
        bindAtt.baseBindAttrib.p_diameter = 8.0;
        bindAtt.gravity = GLKVector3Make(0, 0, 0);
        bindAtt.lifeTime = 2.2;
...
//
    ...
    GLKVector3 beginVelocity = GLKVector3Make(0.0, 0.0, 0.0);;
    glUniform3fv(self.bindObject->uniforms[MoreObjectUniformLocationBeginVelocity], 1,beginVelocity.v);
