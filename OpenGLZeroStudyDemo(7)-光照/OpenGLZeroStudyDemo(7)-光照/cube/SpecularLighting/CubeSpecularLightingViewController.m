//
//  CubeSpecularLightingViewController.m
...
#import "CubeSpecularLightingViewController.h"
#import "CubeSpecularLightingBindObject.h"
#import "CubeManager.h"

@interface CubeSpecularLightingViewController ()
...
@implementation CubeSpecularLightingViewController

-(void)initSubObject{
    //生命周期三秒钟
    __weakSelf
    self.bindObject = [CubeSpecularLightingBindObject new];
    self.bindObject.uniformSetterBlock = ^(GLuint program) {
        ...
        weakSelf.bindObject->uniforms[CubeSpecularLightingUniformLocationModel] = glGetUniformLocation(self.shader.program, "u_model");
        weakSelf.bindObject->uniforms[CubeSpecularLightingUniformLocationInvermodel] = glGetUniformLocation(self.shader.program, "u_inverModel");
        weakSelf.bindObject->uniforms[CubeSpecularLightingUniformLocationDiffusePositon] = glGetUniformLocation(self.shader.program, "lightPos");
        weakSelf.bindObject->uniforms[CubeSpecularLightingUniformLocationDiffuseLightColor] = glGetUniformLocation(self.shader.program, "lightColor");
        weakSelf.bindObject->uniforms[CubeSpecularLightingUniformLocationviewPos] = glGetUniformLocation(self.shader.program, "viewPos");
...
-(void)loadVertex{
    ...
    [self.vertexColor enableVertexInVertexAttrib:CubeSpecularLightingBindAttribLocationVertexColor numberOfCoordinates:3 attribOffset:0];
    ...
    [self.vertexNormal enableVertexInVertexAttrib:CubeSpecularLightingBindAttribLocationNormal numberOfCoordinates:3 attribOffset:0];
    
    GLKVector3 diffColor = GLKVector3Make(1.0, 1.0, 1.0);
    glUniform3fv(self.bindObject->uniforms[CubeSpecularLightingUniformLocationDiffuseLightColor], 1,diffColor.v);
}
...
//        angle = 45;
    ...
    glUniformMatrix4fv(self.bindObject->uniforms[CubeSpecularLightingUniformLocationModel], 1, 0,mode.m);
    bool isSuccess = YES;
    mode = GLKMatrix4InvertAndTranspose(mode,&isSuccess);
    glUniformMatrix4fv(self.bindObject->uniforms[CubeSpecularLightingUniformLocationInvermodel], 1, 0,mode.m);
    
    GLKVector3 diffPostion = GLKVector3Make(5.0, 0.0,0);
    glUniform3fv(self.bindObject->uniforms[CubeSpecularLightingUniformLocationDiffusePositon], 1,diffPostion.v);
    GLKVector3 viewPos = GLKVector3Make(.0, 0.0,2.0);
    glUniform3fv(self.bindObject->uniforms[CubeSpecularLightingUniformLocationviewPos], 1,viewPos.v);
