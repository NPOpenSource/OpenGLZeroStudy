//
//  NotAlphaNotBlendViewController.m
...
#import "NotAlphaNotBlendViewController.h"
#import "NotAlphaNotBlendBindObject.h"
#import "CubeManager.h"
float NANST_planeVertices[] = {
...
float NANST_transparentVertices[] = {
...
@interface NotAlphaNotBlendViewController ()
...
@implementation NotAlphaNotBlendViewController

-(void)initSubObject{
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LESS);
    self.bindObject = [NotAlphaNotBlendBindObject new];
}
...
-(void)loadVertex{
    ...
    for (int i=0; i<vertexNum; i++) {
        float onevertex[5];
        for (int j=0; j<3; j++) {
            onevertex[j]=NANST_planeVertices[i*5+j];
        }
        for (int j=0; j<2; j++) {
            onevertex[j+3]=NANST_planeVertices[i*5+3+j];
        }
    ...
    for (int i=0; i<vertexNum; i++) {
        float onevertex[5];
        for (int j=0; j<3; j++) {
            onevertex[j]=NANST_transparentVertices[i*5+j];
        }
        for (int j=0; j<2; j++) {
            onevertex[j+3]=NANST_transparentVertices[i*5+3+j];
        }
...
-(void)_bindViewMatrix4:(GLBaseBindObject*)bindObject{
    ...
    glUniformMatrix4fv(bindObject->uniforms[NANST_uniform_view], 1, 0,viewMatrix.m);
}
-(void)_bindProjectionMatrix4:(GLBaseBindObject*)bindObject{
    ...
    GLKMatrix4MakePerspective(
                              ...
                              0.1f,
                              100.0f);
    glUniformMatrix4fv(bindObject->uniforms[NANST_uniform_projection], 1, 0,projectionMatrix.m);
}
...
-(void)glkView:(GLKView *)view drawInRect:(CGRect)rect{
    ...
    for (GLuint i = 0; i < 5; i++)
    {
        ...
        glUniformMatrix4fv(self.bindObject->uniforms[NANST_uniform_model], 1, 0,model.m);

        [self.transparentUnit bindtextureUnitLocationAndShaderUniformSamplerLocation:self.bindObject->uniforms[NANST_uniform_Texture]];
        [self.transparentVertex enableVertexInVertexAttrib:NANST_aPos numberOfCoordinates:3 attribOffset:0];
        [self.transparentVertex enableVertexInVertexAttrib:NANST_aTexCoords numberOfCoordinates:2 attribOffset:sizeof(float)*3];
...
-(void)_drawCubeMode:(GLKMatrix4)mode bindObject:(GLBaseBindObject*)bindObject{
    ...
    glUniformMatrix4fv(bindObject->uniforms[NANST_uniform_model], 1, 0,mode.m);
    
    [self.cubeUnit bindtextureUnitLocationAndShaderUniformSamplerLocation:bindObject->uniforms[NANST_uniform_Texture]];
    [self.vertex enableVertexInVertexAttrib:NANST_aPos numberOfCoordinates:3 attribOffset:0];
    [self.vertex enableVertexInVertexAttrib:NANST_aTexCoords numberOfCoordinates:2 attribOffset:sizeof(float)*3];
...
-(void)_drawFloor:(GLBaseBindObject*)bindObject{
    ...
    GLKMatrix4 mode = GLKMatrix4Identity;
    glUniformMatrix4fv(bindObject->uniforms[NANST_uniform_model], 1, 0,mode.m);
     [self.floorUnit bindtextureUnitLocationAndShaderUniformSamplerLocation:bindObject->uniforms[NANST_uniform_Texture]];
    [self.planeVertex enableVertexInVertexAttrib:NANST_aPos numberOfCoordinates:3 attribOffset:0];
    [self.planeVertex enableVertexInVertexAttrib:NANST_aTexCoords numberOfCoordinates:2 attribOffset:sizeof(float)*3];
