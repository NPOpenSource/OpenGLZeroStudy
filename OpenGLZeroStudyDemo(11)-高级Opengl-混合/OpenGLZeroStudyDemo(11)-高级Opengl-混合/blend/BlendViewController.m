...
-(void)initSubObject{
    ...
    self.bindObject = [BlendBindObject new];
}
...
-(void)loadVertex{
    ...
    for (int i=0; i<vertexNum; i++) {
        float onevertex[5];
        for (int j=0; j<3; j++) {
            onevertex[j]=transparentVertices[i*5+j];
        }
        for (int j=0; j<2; j++) {
            onevertex[j+3]=transparentVertices[i*5+3+j];
        }
...
-(void)createTextureUnit{
    ...
    [self.transparentUnit setImage:[UIImage imageNamed:@"window.png"] IntoTextureUnit:GL_TEXTURE2 andConfigTextureUnit:nil] ;
...
-(void)glkView:(GLKView *)view drawInRect:(CGRect)rect{
   ...
   glEnable(GL_DEPTH_TEST);
    ...
    {
        GLKMatrix4 model =[self _getModeMatrix4Location:[self _getWindowsLocation][i]];
        glUniformMatrix4fv(self.bindObject->uniforms[ST_uniform_model], 1, 0,model.m);

        [self.transparentUnit bindtextureUnitLocationAndShaderUniformSamplerLocation:self.bindObject->uniforms[ST_uniform_Texture]];
        [self.transparentVertex enableVertexInVertexAttrib:ST_aPos numberOfCoordinates:3 attribOffset:0];
        [self.transparentVertex enableVertexInVertexAttrib:ST_aTexCoords numberOfCoordinates:2 attribOffset:sizeof(float)*3];
